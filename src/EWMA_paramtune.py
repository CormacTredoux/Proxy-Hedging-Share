...
def run_alpha_analysis(mode='posthoc', lookback=LOOKBACK, window_size=WINDOW_SIZE):
    ...
    if mode == 'pretest':
        ...
        output_subdir = f"EWMA_Alpha_Pretest_Lookback{lookback}"
    else:
        test_start = start_day
        test_size = testing_size
        analysis_name = "Post-hoc Analysis"
        output_subdir = "EWMA_Alpha_Analysis"
    
    alphas = np.arange(0.01, 0.1, 0.0005).round(4)
    ...
    for alpha in tqdm(alphas, desc=f"Processing alpha values ({mode})"):
        try:
            model = EWMAModel(
                data_file=data_file,
                fund_name=fund_name,
    ...
    #### save results
    results_file = output_dir / f'ewma_alpha_{mode}_results.xlsx'
    ...
    return results_df, best_abs_te['alpha']
...
def create_analysis_plots(results_df, output_dir, analysis_name, mode):
    ...
    fig.suptitle(f'EWMA Alpha Parameter Analysis (Window Size={WINDOW_SIZE})\n{analysis_name}', fontsize=14, y=0.995)
    ...
    ax1 = axes[0, 0]
    ax1.plot(alphas, results_df['mean_tracking_error'], linewidth=2, color='blue', marker='o', markersize=3)
    ...
    ax2 = axes[0, 1]
    ax2.plot(alphas, results_df['abs_mean_tracking_error'], linewidth=2, color='red', marker='o', markersize=3)
    ax2.axvline(x=best_alpha, color='black', linestyle=':', linewidth=2, label=f'Optimal: {best_alpha:.2f}')
    ...
    ax3 = axes[1, 0]
    ax3.plot(alphas, results_df['sd_tracking_error'], linewidth=2, color='orange', marker='o', markersize=3)
    ax3.axvline(x=best_alpha, color='black', linestyle=':', linewidth=2)
    ax3.set_ylabel('SD Tracking Error', fontsize=11)
    ax3.grid(True, alpha=0.3)
    
    # middle right: mean replication error
    ax4 = axes[1, 1]
    ax4.plot(alphas, results_df['mean_replication_error'], linewidth=2, color='green', marker='o', markersize=3)
    ax4.axhline(y=0, color='black', linestyle='--', linewidth=1, alpha=0.5)
    ax4.axvline(x=best_alpha, color='black', linestyle=':', linewidth=2)
    ax4.set_ylabel('Mean Replication Error', fontsize=11)
    ax4.grid(True, alpha=0.3)
    
    # bottom left: absolute mean replication error
    ax5 = axes[2, 0]
    ax5.plot(alphas, results_df['abs_mean_replication_error'], linewidth=2, color='purple', marker='o', markersize=3)
    ax5.axvline(x=best_alpha, color='black', linestyle=':', linewidth=2)
    ax5.set_xlabel('Alpha Parameter', fontsize=11)
    ax5.set_ylabel('Absolute Mean Replication Error', fontsize=11)
    ax5.grid(True, alpha=0.3)
    
    # bottom right: sd replication error
    ax6 = axes[2, 1]
    ax6.plot(alphas, results_df['sd_replication_error'], linewidth=2, color='brown', marker='o', markersize=3)
    ax6.axvline(x=best_alpha, color='black', linestyle=':', linewidth=2)
    ax6.set_xlabel('Alpha Parameter', fontsize=11)
    ax6.set_ylabel('SD Replication Error', fontsize=11)
    ax6.grid(True, alpha=0.3)
    
    plt.tight_layout()
    
    plot_file = output_dir / f'ewma_alpha_{mode}_plots.png'
...
if __name__ == '__main__':
    if POSTHOC == 0:
        results_df, best_alpha = run_alpha_analysis(mode='pretest', lookback=LOOKBACK)
        plt.show()
        
    elif POSTHOC == 1:
        results_df, _ = run_alpha_analysis(mode='posthoc', lookback=LOOKBACK)
        plt.show()
        
    elif POSTHOC == 2:
        pretest_df, best_alpha = run_alpha_analysis(mode='pretest', lookback=LOOKBACK)
        posthoc_df, _ = run_alpha_analysis(mode='posthoc', lookback=LOOKBACK)
        plt.show()
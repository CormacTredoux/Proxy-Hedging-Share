...
def init_thread(proxies, fund, proxies_train, fund_train, cols, w_size, test, s_test, s_train, rate, fpath, fname, alph):
    #### initialize thread with shared data
    global Proxies, Fund, Proxies_train, Fund_train, proxy_cols, window_size, testing, start_test, start_train, r, file_path, fund_name, alpha
    Proxies = proxies
    Fund = fund
...
def run_single_simulation(sim_idx):
    ...
    model = EWMAModel.__new__(EWMAModel)
...
if __name__ == '__main__':
    ...
    output_dir = Path(PROJECT_ROOT) / "results" / fund_name / "-".join(proxy_cols_main) / "EWMA_EQUI"
    ...
    if n_threads == 1:
        ...
        for i in range(n_sim):
            init_thread(Proxies_main, Fund_main, Proxies_train_main, Fund_train_main, 
                       ...
                       start_train_main, r_main, file, fund_name, alpha_main)
        ...
        with Pool(processes=n_threads, initializer=init_thread, 
                  initargs=(Proxies_main, Fund_main, Proxies_train_main, Fund_train_main,
                           proxy_cols_main, window_size_main, testing_main, start_test_main,
                           start_train_main, r_main, file, fund_name, alpha_main)) as pool:
            results = []
    ...
    plt.title(f"Distribution of Final Tracking Errors (EWMA, α={alpha_main})", fontsize=14)
    plt.grid(True, linestyle='--', alpha=0.6)
    plt.axvline(mean_final, color='red', linestyle='dashed', linewidth=2, label=f"Mean={mean_final:.4f}")
    plt.legend(fontsize=10)
    plt.tight_layout()

    plot_file = output_dir / f"EWMA_final_tracking_error_alpha{alpha_main}.png"
    ...
    plt.ylabel("Density", fontsize=12)
    plt.title(f"Distribution of Average Tracking Errors (EWMA, α={alpha_main})", fontsize=14)
    plt.grid(True, linestyle='--', alpha=0.6)
    plt.axvline(mean_avg, color='red', linestyle='dashed', linewidth=2, label=f"Mean={mean_avg:.4f}")
    plt.legend(fontsize=10)
    plt.tight_layout()

    plot_file = output_dir / f"EWMA_average_tracking_error_alpha{alpha_main}.png"
    plt.savefig(plot_file, dpi=200)
    plt.show()
    ...
    excel_file = output_dir / f"EWMA_sim_results_QQQxEQUI_alpha{alpha_main}.xlsx"
